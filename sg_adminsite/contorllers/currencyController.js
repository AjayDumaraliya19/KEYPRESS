...
const insertCurrency = async (req, res) => {
    ...
    try {
        ...
        const checkData = await getDataByField(connection, 'currency', `Name = '${req.body.Name}' and IsDelete = 0` )
...
const updateCurrency = async (req, res) => {
    let connection = mysql.createPool(config);
    try {
        const checkData = await getDataByField(connection, 'currency', `CurrencyId = '${req.params.CurrencyId}' and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
        ...
        const updating = await updateData(connection, 'currency', req.params, req.body);
        if (updating.error) {
            return res.status(400).send({StatusCode: 1})
        }
...
const deleteCurrency = async (req, res, next) => {
    let connection = mysql.createPool(config);
    try {
        const checkData = await getDataByField(connection, 'currency', `CurrencyId = '${req.params.CurrencyId}' and IsActive = 1 and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
            return res.status(400).send({StatusCode: 21})
        }
        const deleting = await updateData(connection, 'currency', req.params, { ModifiedBy: req.userData.userId, ModifiedOn: await wutc(), IsDelete: 1 });
        if (deleting.error) {
...
const listCurrency = async (req, res) => {
    ...
    try {
        ...
        const list = await getData(connection, 'currency', fields, `IsDelete = 0`, req.body.Limit, req.body.Page, req.body);
        if (list.error) {
            return res.status(401).send({StatusCode: 1})
        }
        res.status(200).send({
