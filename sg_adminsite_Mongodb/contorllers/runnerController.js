...
const updateRunner = async (req, res) => {
  ...
  try {
    const checkData = await getDataByField(
      connection,
      tableName,
      `RunnerId = '${req.params.RunnerId}' and IsDelete = 0`
    );
...
const deleteRunner = async (req, res, next) => {
  ...
  try {
    const checkData = await getDataByField(
      connection,
      tableName,
      `RunnerId = '${req.params.RunnerId}' and IsActive = 1 and IsDelete = 0`
    );
    if (checkData.data[0].length == 0) {
      return res.status(400).send({ StatusCode: 19 });
    }
...
/* ------------------------- List Runner By game Id ------------------------- */
const listRunnerByGameId = async (req, res) => {
  let connection = mysql.createPool(config);
  try {
    const fields = `RunnerId,GameId,Name,BackOdd,LayOdd,GroupId,Rcode,IsActive+0 AS IsActive,CreatedOn`;
    const list = await getDataByField(
      connection,
      tableName,
