...
const insertUpdatePermission = async (req, res) => {
    ...
    try {
        for (let i of permissionData) {
            if (await ifExist(connection,tableName, `PageId = ${i.PageId} AND  RoleId = ${i.RoleId}`)) {
                let updating = await updateData1(connection, tableName, `PageId = ${i.PageId} AND  RoleId = ${i.RoleId}`, {Action: i.Action, ModifiedBy: req.userData.userId, ModifiedOn: await wutc()});
            } else {
...
const insertPermission = async (req, res) => {
    ...
    try {
        ...
        const checkData = await getDataByField(connection, 'bopermission', `RoleId = ${req.body.RoleId} and PageId = ${req.body.PageId} and IsDelete = 0`)
...
const updatePermission = async (req, res) => {
    let connection = mysql.createPool(config);
    try {
        const checkData = await getDataByField(connection, tableName, `PermissionId = '${req.params.PermissionId}' and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
        ...
        const updating = await updateData(connection, 'bopermission', req.params, req.body);
        if (updating.error) {
            return res.status(400).send({StatusCode: 1})
        }
        res.status(200).send({StatusCode: 6})
...
const deletePemission = async (req, res, next) => {
    let connection = mysql.createPool(config);
    try {
        const checkData = await getDataByField(connection, tableName, `PermissionId = '${req.params.PermissionId}' and IsActive = 1 and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
            return res.status(400).send({StatusCode: 20})
        }
        const deleting = await updateData(connection, 'bopermission', req.params, { ModifiedBy: req.userData.userId, ModifiedOn: await wutc(), IsDelete: 1 });
        if (deleting.error) {
...
const permissionByRoleId = async (req, res) => {
    ...
    try {
        ...
        const list = await getDataWithoutPagi(connection, tableName, fields, `RoleId = ${req.body.RoleId} and IsActive = 1 and IsDelete = 0`);
        if (list.error) {
            return res.status(401).send({StatusCode: 1})
        }
        res.status(200).send({
