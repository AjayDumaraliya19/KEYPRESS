...
const insertRole = async (req, res) => {
    ...
    try {
        ...
        const checkData = await getDataByField(connection, 'boroles', `Name = '${req.body.Name}' and IsDelete = 0`)
...
const updateRole = async (req, res) => {
    ...
    try {
        ...
        const checkData = await getDataByField(connection, tableName, `RoleId = '${req.params.RoleId}' and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
        ...
        const updating = await updateData(connection, 'boroles', req.params, req.body);
        if (updating.error) {
            return res.status(400).send({StatusCode: 1})
        }
...
const deleteRole = async (req, res) => {
    ...
    try {
        ...
        const deleting = await updateData(connection, 'boroles', req.params, { ModifiedBy: req.userData.userId, ModifiedOn: await wutc(), IsDelete: 1 });
        if (deleting.error) {
...
const listRole = async (req, res) => {
    ...
    try {
        ...
        const list = await getData(connection, 'boroles', fields, `IsDelete = 0`, req.body.Limit, req.body.Page, req.body);
        if (list.error) {
            return res.status(401).send({StatusCode: 1})
        }
