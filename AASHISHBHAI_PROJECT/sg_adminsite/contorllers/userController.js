...
const userCreate = async (req, res) => {
    ...
    try {
        ...
        const checkData = await getDataByField(connection, tableName, `Username = '${req.body.Username}' and IsDelete = 0`, 'Username')
        if (checkData.data[0].length != 0) {
            return res.status(400).send({
                StatusCode: 9
            })
        }
        const checkRole = await getDataByField(connection, tableName1, `RoleId = '${req.body.RoleId}' and IsActive = 1 and IsDelete = 0`, 'RoleId')
...
const login = async (req, res, next) => {
    ...
    try {
        ...
        if (body.Username && body.Password) {
            ...
            let params = [body.Username, encryptdPassword, ((roleId !== 1 && roleId !== 4) ? res.status(401).send({StatusCode: 4 }) : roleId )];
...
const listUser = async (req, res) => {
    ...
    try {
        ...
        const list = await joinsTwoTb(connection, tableName, 'boroles', field, `B.RoleId = A.RoleId`, `A.IsDelete = 0`, req.body.Limit, req.body.Page, req.body)
        if (list.error) {
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        if (await ifExist(connection, 'bousers', `UserId = ${JSON.parse(separateArray[0])} AND Password = '${encryptdPassword}'`)) {
            const encryptdNewPassword = await Eencrypted(req.body.NewPW);
            let updating = await updateData1(connection, tableName, `UserId = ${JSON.parse(separateArray[0])}`, {
