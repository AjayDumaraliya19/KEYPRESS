...
const insertPage = async (req, res) => {
    ...
    try {
        ...
        const checkData = await getDataByField(connection, 'bopages', `Name = '${req.body.Name}' and IsDelete = 0`)
...
const updatePage = async (req, res) => {
    let connection = mysql.createPool(config);
    try {
        const checkData = await getDataByField(connection, tableName, `PageId = ${req.params.PageId} and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
        ...
        const updating = await updateData(connection, 'bopages', req.params, req.body);
        if (updating.error) {
            return res.status(400).send({ StatusCode: 1 })
        }
...
const deletePage = async (req, res, next) => {
    ...
    try {
        ...
        const deleting = await updateData(connection, 'bopages', req.params, { ModifiedBy: req.userData.userId, ModifiedOn: await wutc(), IsDelete: 1 });
        if (deleting.error) {
...
const listPage = async (req, res) => {
    ...
    try {
        ...
        const list = await getData(connection, 'bopages', fields, `IsDelete = 0`, req.body.Limit, req.body.Page, req.body);
        if (list.error) {
            return res.status(401).send({ StatusCode: 1 })
        }
...
const listPageWithoutPagination = async (req, res) => {
    ...
    try {
        ...
        if (req.params.list == 'active') {
            list = await getDataWithoutPagi(connection, 'bopages', fields, `IsActive = 1 and IsDelete = 0`);
            if (list.error) {
                return res.status(401).send({ StatusCode: 1 })
            }
        }
        ...
        if (req.params.list == 'all') {
            list = await getDataWithoutPagi(connection, 'bopages', fields, `IsDelete = 0`);
            if (list.error) {
                return res.status(401).send({ StatusCode: 1 })
            }
        }
...
module.exports = { insertPage, updatePage, deletePage, listPage, listPageWithoutPagination };