...
const insertGame = async (req, res) => {
    ...
    try {
        ...
        let params = [Name, Code, Image, DisplayOrder, Description, Rules, StreamUrl, HowToPlayUrl, IsExposure, IsSound, GameSec, CardSec, Theme, req.userData.userId]
...
const updateGame = async (req, res) => {
    ...
    try {
        const checkData = await getDataByField(connection, tableName, `GameId = '${req.params.GameId}' and IsDelete = 0`,'Code')
        ...
        const updating = await updateTwoTbl(connection, tableName, 'gamedetail', req.params, req.body)
        if (updating.error) {
...
const updateGameDetails = async (req, res, next) => {
    let connection = mysql.createPool(config);
    try {
        const checkData = await getDataByField(connection, tableName, `GameId = '${req.params.GameId}' and IsDelete = 0`)
        if (checkData.data[0].length == 0) {
            return res.status(400).send({
                StatusCode: 12
            })
        ...
        const updating = await updateData(connection, 'gamedetail', req.params, req.body);
        if (updating.error) {
            return res.status(400).send({
                StatusCode: 1
            });
...
const gameList = async (req, res) => {
    ...
    try {
        ...
        const list = await joinsTwoTb(connection, tableName, 'gamedetail', fields, `B.GameId = A.GameId`, `A.IsDelete = 0`, req.body.Limit, req.body.Page, req.body);
        if (list.error) {
...
const gameWithoutPagination = async (req, res) => {
    ...
    try {
        ...
        const list = await getDataWithoutPagi(connection, tableName, fields, `IsActive = 1 and IsDelete = 0`);
        if (list.error) {
            return res.status(401).send({
                StatusCode: 1
            })
...
const gameDetailsByGameCode = async (req, res) => {
    ...
    try {
        ...
        const gameData = await joinsTwoTbl(connection, tb1, tb8, fields, `A.GameId = B.GameId`, `Code = '${req.body.Code}'`);
        ...
        const runnerData = await getDataByField(connection, tb6, `GameId = '${gameData.data.data[0].gi}' and IsActive = 1 and IsDelete = 0`, `RunnerId as rui, Name as rn, GroupId as gri, Rcode as rc`);
...
const LastTenWinners = async (req, res) => {
    ...
    try {
        ...
        const gameData = await getDataByField(connection, tableName, `Code = '${req.body.Code}' and IsActive = 1 and IsDelete = 0`, 'GameId')
...
const CancelRoundbygameCode = async (req, res, next) => {
    ...
    try {
        ...
        axios.post(process.env.cru, postData)
            .then((response) => {
