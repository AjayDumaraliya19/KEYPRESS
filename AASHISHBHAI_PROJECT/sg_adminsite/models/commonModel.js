...
exports.getData = async (connection, tableName, fields, whereCondition, limit, page, body) => {
	let obj = {
		error: null,
		data: null
	};
	try {
		...
		if (body.Search && body.Search.Searchtext) {
		  ...
		  const searchColumns = body.Search.Column.map(col => `${col} LIKE ?`).join(" OR ");
...
exports.insertData = async (connection, tableName, insertData) => {
	...
	exports.updateData = async (connection, tableName, query, updateData) => {
		...
		try {
			let sql = `UPDATE ${tableName} set ? where ${Object.keys(query)[0]} = '${Object.values(query)[0]}'`;
...
exports.updateTwoTbl = async (connection, tb1, tb2, whereCondition, updateData) => {
	let obj = {
		error: null,
		data: null
	};
	try {
		let sql = `UPDATE ${tb1} AS t1 JOIN ${tb2} AS t2 ON t1.GameId = t2.GameId SET ? WHERE  t1.${Object.keys(whereCondition)[0]} = '${Object.values(whereCondition)[0]}'`;
		const data = await connection.query(sql, [updateData]);
		obj.data = data;
		return (obj);
	} catch (error) {
