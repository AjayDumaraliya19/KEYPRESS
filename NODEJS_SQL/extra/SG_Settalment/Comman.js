...
exports.addTransaction = async (connection, TransactionObj) => {
    const addTransaction = await insertData(connection, 'transactions', TransactionObj);    
    return addTransaction.data[0].insertId;
}

exports.updateTransaction = async (connection, res, TransactionId, objupdateTransaction) => {
    const updateTransaction = await updateData1(connection, tb2, `TransactionId = ${TransactionId}`, objupdateTransaction);
...
exports.Credit = async (req, pdata) => {
    try {
        const sessionId = await Eencrypted(pdata.Pldata.plid + '/' + pdata.Pldata.pln + '/' + pdata.Pldata.plst)
        const partnerKey = await Eencrypted(pdata.Ptdata.ptid + '/' + pdata.Ptdata.ptn + '/' + pdata.Ptdata.plsk)
...
exports.rollback = async (authorization, req, partnerData) => {
    try {
        ...
        const key = partnerData.data.data[0].PartnerId + '/' + partnerData.data.data[0].PartnerName + '/' + partnerData.data.data[0].SecretKey
