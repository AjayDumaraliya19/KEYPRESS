...
async function updateplayer(
  ...
  try {
    ...
    await loggerdata(
      `${timejson.TransactionObj} : Insert Tran ${JSON.stringify(
        TransactionObj
      )}`
    );
    await loggerdata(`Res Insert Tran ${objDebitTransaction}`);
    ...
    if (Creditres.StatusCode === 1 || Creditres.StatusCode === -1) {
      ...
      await loggerdata(`Res rollback Tran ${objrollbacktransaction}`);
      await loggerdata(`Request rollback ${JSON.stringify(CreditReq)}`);
      await loggerdata(`response rollback ${JSON.stringify(rollbackres)}`);
    } else if (Creditres.StatusCode === 0) {
      await loggerdata(`Update Tran ${objDebitTransaction}`);
      await loggerdata(`Update Tran ${JSON.stringify(objupdateTransaction)}`);
    }
  }
  await loggerdata(
    `===== PlayerId : ${playerdata.pdata.Pldata.plid}  PlayerName : ${playerdata.pdata.Pldata.pln} =====`
